...
ClinicalGNN-LLM Agent는 실제 ICU 임상 데이터(MIMIC-III)를 활용한 AI Agent 서비스입니다.

- 환자 ID 입력 시 **실제 진단 정답**과 **GNN 유사도 기반 예측 질병**을 함께 표시
- 예측 결과와 실제 진단의 **일치 여부(✅)** 를 시각적으로 구분
- MIMIC-III Discharge Summary RAG 검색으로 유사 임상 노트 제공
- 멀티턴 대화로 이전 환자 문맥 유지 ("이 환자 폐렴이랑 관련 있어?")
- Flask 웹 UI + 햄버거 메뉴에서 전체 환자 목록 조회 및 클릭 질의
...
| 입력 예시 | 동작 |
|-----------|------|
| `환자 1197번 예측해줘` | 실제 진단 정답 + GNN Top-5 예측 + 바이탈 분석 |
| `환자 10006번 질병 예측해줘` | 실제 진단 정답 + GNN Top-5 예측 + 바이탈 분석 |
| `환자 1197번 예측하고 질병 설명도 해줘` | GNN 예측 + RAG 임상 노트 검색 동시 수행 |
...
1. 좌측 상단 **☰ 햄버거 버튼** 클릭
2. 사이드바에서 전체 환자 ID 목록 확인 (페이지네이션, 15명씩)
3. ID 검색창으로 특정 환자 빠르게 찾기
4. 환자 카드 클릭 → 입력창에 질의 자동 채움 후 사이드바 닫힘
...
【GNN 유사도 기반 예측 질병 Top-5】
  1위: [4280] Congestive heart failure   (유사도 0.4814) ✅ 정답 일치
  2위: [5849] Acute kidney failure        (유사도 0.4582)
  3위: [2449] Hypothyroidism              (유사도 0.4580)
  4위: [5859] Chronic kidney disease      (유사도 0.4578)
  5위: [25000] Diabetes mellitus          (유사도 0.4285) ✅ 정답 일치
...
| `gnn_node` | Tool A 호출 — 실제 진단 정답 + GNN 코사인 유사도 기반 Top-5 예측, 정답 일치 여부(matched) 표시 |
| `rag_node` | Tool B 호출 — 쿼리 정제(한→영) 후 FAISS 벡터 검색, 유사도 필터링 |
| `retry_node` | RAG 실패 시 LLM으로 쿼리 재작성 후 rag_node 재시도 (최대 2회, loop) |
...
- `rag_node` → 결과 있음 → `report_node` / 결과 없음 + 재시도 가능 → `retry_node` / 한계 초과 → `report_node`
- `retry_node` → `rag_node` (loop, 최대 2회)
...
ANALYTICS_PATH=C:/path/to/newdata/newdata_analytics_master.json
...
- `newdata_analytics_master.json`에서 **실제 진단 정답(actual_ground_truth)** 로드
- 예측 결과와 정답 ICD 코드 비교 → `matched` 필드로 일치 여부 표시
...
- **retry loop**: 검색 실패 시 `retry_node`에서 쿼리 재작성 후 최대 2회 재시도
  - 예) `"diabetes mellitus type 2"` → `"diabetes"`
- 검색 결과에 유사도 점수 포함 출력
...
- `rag_retry_count` — RAG retry loop 제어용 카운터 (매 턴 시작 시 0으로 리셋)
...
- 헤더 좌측 **☰ 버튼** 클릭 시 사이드바 슬라이드 오픈 (X 애니메이션 전환)
...
├── .env                    # API Key (Git 제외)
├── agent/
│   ├── graph.py            # LangGraph StateGraph 정의 (retry loop 포함)
│   └── state.py            # AgentState / ConversationTurn 스키마
├── tools/
│   └── tools.py            # GNN / RAG Tool 정의 (정답 데이터 포함)
├── middleware/
...
- **GNN 예측 신뢰도**: 코사인 유사도가 0.5 미만인 경우가 많아 예측 정확도 제한적
...
- **retry 횟수 제한**: RAG 재시도 최대 2회로 고정 (환경변수 미노출)
